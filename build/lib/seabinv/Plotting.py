...
class PlotFromStorage(object):
    ...
    def init_filelists(self):
        ...
        if len(set(size)) == 1:
            self.modfiles, self.likefiles, self.misfiles, self.noisefiles = files
        else:
...
# Considering weighted models.
    ...
    @staticmethod
    def _plot_bestmodels(bestmodels, dep_int=None, x=None, y=None):
        ...
        for i, model in enumerate(models):
            rsl, age = singlemodels[model]

            ax.plot(age, rsl.T, color=colors[i], label=model,
                    ls=ls[i], lw=lw[i])
    ...
    @staticmethod
    def _plot_bestmodels_hist(models, dep_int=None, x=None, y=None):
        """
        ...
        if dep_int is None:
            ...
            depbins = np.arange(dep_int[0]-interp, maxdepth + 2*interp, interp)  # interp km bins
            # nbin = np.arange(0, maxdepth + interp, interp)  # interp km bins

        # get interfaces, #first
        models2 = ModelMatrix._replace_age_rsl(models)
        models2 = np.array([model[~np.isnan(model)] for model in models2])
        yinterf = np.array([model[:int(model.size/2)] for model in models2])
        ...
        # initiate plot
        fig, axes = plt.subplots(2, 1, gridspec_kw={'height_ratios': [4, 2]},
                                 sharex=True, figsize=(8.6, 6))
        fig.subplots_adjust(hspace=0.05)
        data2d, xedges, yedges = np.histogram2d(deps_int.flatten(), vss_flatten,
                                				bins=(depbins, vsbins))
        ...
        axes[0].plot(x, y, color='red', lw=1, alpha=0.9)
        # for c, choice in enumerate(['mode']):
        #     rsl, age = singlemodels[choice]
        #     color = colors[c]
        #     axes[0].plot(age, rsl, color=color, lw=1, alpha=0.9, label=choice)

        rsl_mode, age_mode = singlemodels['mode']
        axes[0].legend(loc=3, fontsize=10)

        # histogram for interfaces
        bin_age = np.arange(dep_int[0], maxdepth + interp, interp)
        data = axes[1].hist(yinterf, bins=bin_age, orientation='vertical',
    ...
    def plot_posterior_others(self, final=True, chainidx=0):
        ...
        axes = axes.flatten()
        for i, data in enumerate([likes, misfits, layers]):
            ax = axes[i]
    ...
    def plot_posterior_models1d(self, final=True, chainidx=0, depint=1):
        ...
        dep_int = np.arange(self.priors['age'][0],
                            self.priors['age'][1] + depint, depint)
        targets = Targets.JointTarget(targets=self.targets)
        target = targets.targets[0]
        x_obs, y_obs = target.obsdata.x, target.obsdata.y
        fig, ax = self._plot_bestmodels(models, dep_int, x_obs, y_obs)
    ...
    def plot_posterior_models2d(self, final=True, chainidx=0, depint=1):
        ...
        dep_int = np.arange(self.priors['age'][0]+depint,
                            self.priors['age'][1], depint)
        targets = Targets.JointTarget(targets=self.targets)
        target = targets.targets[0]
        x_obs, y_obs = target.obsdata.x, target.obsdata.y
        fig, axes = self._plot_bestmodels_hist(models, dep_int, x_obs, y_obs)
        # axes[0].set_xlim(self.priors['vs'])
        axes[0].set_ylim(self.priors['rsl'])
...
# Plot moho depth - crustal vs tradeoff
    ...
    def plot_moho_crustvel_tradeoff(self, moho=None, mohovs=None, refmodel=None):
        ...
        for i, model in enumerate(models):
            thisvpvs = vpvs[i]
            vs, h = Model.get_age_rsl(model, thisvpvs, self.mantle)
...
# Plot current models and data fits. also plot best data fit incl. model.
    ...
    def plot_currentmodels(self, nchains):
        ...
        for i, modfile in enumerate(self.modfiles[1][:nchains]):
            ...
            color = color_list[i]
            x_age, y_rsl = Model.get_age_rsl(currentmodel)
            cvs, cdepth = Model.get_stepmodel_from_h(x_age, y_rsl)

            label = 'c%d / %d' % (chainidx, x_age)
...
# save best likelihood model
            ...
            np.savetxt('./results/best_like_model'+str(bestlike)+'.txt', np.column_stack((x_age, y_rsl)))
...
# save best Llikes model with mode layers
        ...
        x_age, y_rsl = Model.get_age_rsl(bestmodel_mode)
        np.savetxt('./results/best_mode_model.txt', np.column_stack((x_age, y_rsl)))
        ...
        for i, modfile in enumerate(modfiles):
            ...
            for n, target in enumerate(targets.targets):
                ...
                if len(targets.targets) > 1:
                    ...
                    ax.plot(xmod, ymod, color=color, alpha=0.5, lw=0.7)
            # np.savetxt('./results/best_model_from_chain'+str(chainidx)+'.txt', np.column_stack((x_age, y_rsl)))
