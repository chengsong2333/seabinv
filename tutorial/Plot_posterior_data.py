...
class PlotPosteriorData(object):
    ...
    def plot_posterior_ph(self, final=True, chainidx=0):
        ...
        axes.plot(targets.targets[0].obsdata.x, targets.targets[0].obsdata.y, 'r')
    ...
    def plot_posterior_rwe(self, final=True, chainidx=0):
        ...
        period = targets.targets[2].obsdata.x  # 
        ...
        phinterval = 0.001
        ph_histmin = phs_flatten.min()-2*phinterval
        ph_histmax = phs_flatten.max()+10*phinterval
        phbins = np.arange(ph_histmin, ph_histmax, phinterval) # some buffer
        ...
        axes.plot(targets.targets[2].obsdata.x, targets.targets[2].obsdata.y, 'r')
    ...
    def save_plots(self):
        ...
        self.savefig(fig2a, 'c_posterior_ph.pdf')

        fig2b = self.plot_posterior_rf()
        self.savefig(fig2b, 'c_posterior_rf.pdf')

        #fig2c = self.plot_posterior_rwe()
        #self.savefig(fig2c, 'c_posterior_rwe.pdf')
...
xsw, _ysw = np.loadtxt('observed/TJN_rdispph.dat').T
xrf, _yrf = np.loadtxt('observed/TJN_prf.dat').T
xrwe, _yrwe = np.loadtxt('observed/TJN_rwe.dat').T
